...
**Vitest**, unit-only for now (no integration/e2e yet). `pnpm test` (`svelte-kit sync && vitest run`) — the `svelte-kit sync` is required so the generated `.svelte-kit/tsconfig.json` exists (Vite 8's oxc transform loads it). Config: `vitest.config.ts` (node environment, `$lib` alias, `src/**/*.test.ts`); tests are colocated next to source. Scope is pure, DB-free logic (`src/lib/server/auth/tokens.test.ts`, `normalize-email.test.ts`). CI runs `pnpm test` after `build`. Integration is planned via `@cloudflare/vitest-pool-workers` (local D1 + a fake `EmailSender`) — not built yet.
