...
OVERVIEWS[101] =
	"A harbor pilot's radio starts picking up boats that were decommissioned years ago.";
OVERVIEWS[102] = 'A ghostwriter is hired to finish a memoir the author never wanted published.';
const TMDB_CATALOG = [
	...SHOWS.filter((s) => s.source !== 'custom'),
	...CATALOG_EXTRAS.map((e, i) => enrich(e, i + 20))
];
...
function TimelineScreen({ items, theme, onSelect }) {
	...
	return (
		<div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }}>
			{grouped.length === 0 && (
				<div
					style={{ color: theme.textMuted, fontSize: 13, padding: '40px 0', textAlign: 'center' }}
				>
...
function SettingsSection({ title, theme, children, 'data-spec-ref': specRef }) {
	return (
		<div data-spec-ref={specRef} style={{ marginBottom: 22 }}>
			...
			<div style={{ background: theme.surfaceAlt, borderRadius: 12, overflow: 'hidden' }}>
				{children}
			</div>
		</div>
	);
	...
	return (
		<div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }}>
			...
			<SettingsSection
				title="Notifications"
				theme={theme}
				data-spec-ref="settings-notifications-section"
			>
				{subs.length === 0 && (
					<SettingsRow theme={theme} last>
...
function SearchScreen({
	...
	const results = trimmed
		? pool.filter((i) => i.title.toLowerCase().includes(trimmed.toLowerCase()))
		: [];
	...
	return (
		<div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }}>
					...
					return (
						...
						>
							...
							<button
								type="button"
								onClick={() =>
									tracked ? openItem(items.find((i) => i.id === r.id)) : onAddFromCatalog(r)
								}
								style={{
									padding: '6px 13px',
									borderRadius: 999,
			...
			{trimmed && results.length === 0 && (
				<div style={{ color: theme.textMuted, fontSize: 13, padding: '8px 0 16px' }}>
					No matches.
				</div>
			)}
...
export default function WatchlistConcept() {
	...
	const continueWatching = items.filter(
		(s) => s.status === 'watching' && computeProgress(s) !== undefined
	);

	const inTab = items.filter((s) =>
		activeTab === 'favorites' ? s.favorite : s.status === activeTab
	);
	...
	const renderDetailBody = () => (
		<>
			...
			>
				<div
					style={{ position: 'absolute', inset: 0, background: theme.scrim, pointerEvents: 'none' }}
				/>
				<button
					type="button"
			...
			<div style={{ padding: '16px 20px 20px', position: 'relative' }}>
				...
				{selected.source === 'custom' ? (
					...
					>
						<p
							style={{ color: theme.textMuted, fontSize: 13, lineHeight: 1.5, margin: '0 0 14px' }}
						>
							No overview, cast, or artwork yet — this was added manually and hasn't been matched to
							a database entry. We'll check periodically and let you review a match if one turns up
							— or you can look for one yourself now.
						</p>
					...
					<>
						...
						<div>
							{selected.seasons[activeSeason].episodes.map((ep) => (
								...
								>
									<div style={{ width: 22, fontSize: 12, color: theme.textMuted, fontWeight: 600 }}>
										{ep.number}
									</div>
									<div style={{ flex: 1 }}>
	...
	return (
      ...
      `}</style>
			...
			>
				...
				<div>
					Viewport: {windowWidth}px — {isDesktop ? 'desktop layout' : 'phone layout'}
				</div>
			</div>
			...
			>
				...
				{screen === 'home' && (
					<div
						className="no-scrollbar"
						style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }}
					>
						{/* Continue watching */}
										...
										>
											<div style={{ width: 120 }}>
												<PosterTile item={item} theme={theme}>
													...
													>
														<ProgressRing
															progress={isDone ? 1 : computeProgress(item)}
															size={34}
															theme={theme}
														/>
														{isDone ? (
												...
												<div
													style={{
														fontSize: 11,
														color: phase ? ACCENT : theme.textMuted,
														marginTop: 2,
														fontWeight: phase ? 700 : 400
													}}
												>
													{phase
														? 'Completed!'
														: `S${season.number} · E${nextEp ? nextEp.number : season.episodes.length}`}
												</div>
											</div>
										</div>
									);
						...
						<div
							data-spec-ref="list-type-genre-year-filters"
								...
								>
									...
									{availableYearBuckets.map((b) => (
										<button
											key={b}
											type="button"
											onClick={() => setYearFilter(b)}
											style={chip(yearFilter === b)}
										>
											{b}
										</button>
									))}
				...
				{/* Timeline screen */}
				{screen === 'timeline' && (
					<TimelineScreen items={items} theme={theme} onSelect={openItem} />
				)}

				{/* Settings screen */}
				{screen === 'settings' && (
					<SettingsScreen theme={theme} themeMode={themeMode} setThemeMode={setThemeMode} />
				)}
