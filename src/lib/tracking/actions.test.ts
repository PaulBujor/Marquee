...
describe('episodesToMark', () => {
	...
	// MRQ-210: a weekly show that premiered two months ago with only 5 of 10 episodes actually out.
	...
	// that haven't aired. Mirrors `hasSufficientEpisodeData`'s equivalent null-vs-false fix (#179 /
	// MRQ-194) — the two must never disagree about whether a show is markable.
	...
	// Option B (MRQ-210): TMDB is trusted as accurate once it reports a show finished, even when it
	// never backfilled per-episode rows for a season — a real, permanent data gap, not sync lag. The
	// fallback only fires for an *explicit* `false`, never `null`, so it stays distinct from the
	// unknown-status case above.
