<script lang="ts">
	...
	/**
	 * One credited person on a custom entry: their role and name as a single field, the character
	 * they played when the role is cast, and type-ahead against the provider's people.
	 *
	 * The search is a **spelling aid, not a link**. Picking someone fills in the canonical name and
	 * records who was meant, but the person is still stored as this user's own private row — nothing
	 * is written into the shared catalog and nothing is reconciled against it on sync. Offline it's
	 * plain text input: whatever the user types stands, because an entry authored on a plane has to
	 * work the same as one authored at home.
	 */
	interface Props {
